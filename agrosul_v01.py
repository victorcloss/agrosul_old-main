...
def atualiza_dados_dia(n_intervals):
    ...
    return [Total, 
            ...
            {'margin-right': '5px', 'border': '0px black solid'}, {'margin-right': '5px', 'border': '0px black solid'}]
...
def atualiza_historico(start_date, end_date, ordem, _):
    if not start_date or not end_date:
        raise PreventUpdate
    df_resumo, df_dados_range = get_data_range(dataset, start_date, end_date)
    if df_resumo.empty:
        return html.H5('Sem dados no período'), {}, {'display': 'none'}
    else:
        if ordem == 'ordem alfabética':
            df_aux = df_resumo.sort_values(by=['Integrado'])
        if ordem == 'nota':
            df_aux = df_resumo.sort_values(by=['Nota'], ascending=False)
        if ordem == 'volume':
            df_aux = df_resumo.sort_values(by=['Patas Total'], ascending=False)
        trace_A = go.Bar(y=df_aux['Integrado'] + ':  ' + df_aux['Nota'].astype(str), x=df_aux['PawA'],
                         name='PawA', marker={'color': colors['PawA']}, width=.4, orientation='h')
        trace_B = go.Bar(y=df_aux['Integrado'] + ':  ' + df_aux['Nota'].astype(str), x=df_aux['PawB'],
                         name='PawB', marker={'color': colors['PawB']}, width=.4, orientation='h')
        trace_C = go.Bar(y=df_aux['Integrado'] + ':  ' + df_aux['Nota'].astype(str), x=df_aux['PawC'],
                         name='PawC', marker={'color': colors['PawC']}, width=.4, orientation='h')
        style = {'margin-bottom': '10px',
                 'margin-top': '10px',
                 'margin-left': '30px',
                 'margin-right': '20px',
                 'height': len(df_resumo) * 60 if len(df_resumo) > 6 else 7 * 60,
                 'width': 1000,
                 'border': '0px gray solid',
                 'borderRadius': '15px',
                 'ticksuffix': "  "}
        data_integrados = [trace_A, trace_B, trace_C]
