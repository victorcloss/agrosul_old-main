...
def create_resumo(excel_path, df):
    ...
    data_datetime = pd.to_datetime(df_carga['Unnamed: 0'].iloc[6], origin='1899-12-30', unit='D')
    date_str = data_datetime.strftime('%d/%m/%Y')
    #columns = 'id', 'equipamento_id', 'data', 'hora', 'frame', 'chicken', 'paw_a',
...
def update_data(df, db_path, table_name):
    ...
    today = '2024-08-16'
    query = f"SELECT * FROM {table_name} WHERE data = '{today}' AND hora > '{last_time}'"
...
# df_dados columns = (['data, 'hora', 'Frame', 'chicken', 'paw_a', 'paw_b', 'paw_c', 'PawX',
...
def atualiza_dados_dia(n_intervals):
    global df_dados, table_name
    df_dados = update_data(df_dados, db_path, table_name)
    HH = pd.to_datetime(df_dados['hora'], format='%H:%M:%S').dt.strftime('%H:%M')
