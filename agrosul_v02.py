...
# Function to insert a DataFrame into the 'integrados' table
...
def get_past_days(n_days=10):
    ...
    past_days =  [(today_time - timedelta(days=i)).date() for i in range(n_days)]
    df_past_days = pd.DataFrame([])

    for i in range(len(past_days)):
        df_day = get_resumo_dia(past_days[i])
        df_day.drop([ 'unidade_id', 'name'], axis=1, inplace=True)
        if df_day.empty:
            df_day.loc[0,:] =[past_days[i], 0, 0, 0, 0, 0]
            #print(df_day)
        #else:
            #print('\n\n\n\n')
          #  print(df_day)
            #df_day = df_day.groupby('date').sum()
          #  print('\n\n\n\nAfter goroup by date')
          #  print(df_day)
            


        df_past_days = pd.concat([df_past_days, df_day], ignore_index=True) 

        #print(df_past_days)
        #print('\n\n')
    #print(df_past_days)
    # Group by 'date' and sum the values for other columnss
    df_past_days = df_past_days.groupby('date').sum()
    #print(df_past_days.index)
    return df_past_days
        ...
        dcc.Tab(  # inicio aba historico por periodo
            dbc.Card([
                dbc.Row([
                    ...
                    dbc.Col([
                        dcc.Dropdown(id='integrado-seletor', placeholder="Selecione um integrado")
...
def atualiza_dados_dia(n_intervals):
    ...
    # adicionar IF tela pequena dtick = 2*dtick
    ...
    # ATENÇÃO -> ele não precisa recalcular esse df_last_days toda a hora, só a do dia de hoje
    
    #print('\n\n In fuction \n\n')
    #print(df_last_days)
    df_last_days.iloc[-1,:] =[0, Total, A, B, C]
    #print(df_last_days)

    trace_A = go.Bar(y=df_last_days['paw_a'], x=df_last_days.index#['date']
                         ,name='Patas A', marker={'color': colors['paw_a']})
    trace_B = go.Bar(y=df_last_days['paw_b'], x=df_last_days.index#['date']
                         ,name='Patas B', marker={'color': colors['paw_b']})
    trace_C = go.Bar(y=df_last_days['paw_c'], x=df_last_days.index#['date']
                         ,name='Patas C', marker={'color': colors['paw_c']})
    trace_T = go.Bar(y=df_last_days['chicken'], x=df_last_days.index#['date']
...
@dash_app.callback(
    ...
    Output("integrado-seletor", "options"),
    [Input("historico-intervalo", "start_date"), Input("historico-intervalo", "end_date")],
    Input('historico-ordem', 'value'),  # Just a dummy input to trigger the callback once at the start
...
def atualiza_historico(start_date, end_date, ordem, _):
    ...
    if df.empty:
        ...
        return html.H5('Dados encontrados'), figure, style, df['name']
